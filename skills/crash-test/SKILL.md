...
判出 Never 也分不清到底是「屏障挡住了」还是「这个模式本来就撞不上」。
没配对的 Never，脚本直接拒。

配对**按文件名认**：`x.litmus` 的对照组要叫 `x-<后缀>.litmus`（惯例是 `x-nofence.litmus`）
并声明 Sometimes。全局有几条 Sometimes 不算数——它们回答的不是这一条的问题。
...
**`--selftest` 会跑一个故意失败的 payload**，看 harness 认不认得出失败。
认不出就说明它会把失败当成成功，这时候 harness 自己会报错。

找不到能读的内核时，脚本会失败并给出两条路（设 `SINGLEFS_KERNEL=`，
或者给 `/boot/vmlinuz-*` 加读权限）。**它不会偷偷退回软件模拟**——
那样慢到没法用，却看着像在跑。
...
- **判定结果读不到，这一轮就作废**，绝不能当成通过。`lkmm.sh` 读不到 `Observation`、
